            ...
            if task_id not in task_storage:
                ...
                if os.path.exists(run_sh_file):
                    try:
                        with open(run_sh_file, "r") as f:
                            ...
                            model_match = (
                                run_script.split("--model ")[1].split(" ")[0]
                            ...
                            if "--engine-backend " in run_script:
                                engine_backend = run_script.split("--engine-backend ")[
                                    1
                                ].split(" ")[0]
                        ...
                        print(
                            f"Error extracting model info from run.sh for {task_id}: {str(e)}"
                        )
                ...
                try:
                    with open(result_file, "r") as f:
                        ...
                        task_storage[task_id]["result"] = result_data.get(
                    ...
                    task_storage[task_id][
                        "error"
                    ] = f"Failed to load output file: {str(e)}"
...
def run_story_generation(task_id, prompt, model, api_keys):
    ...
    monitoring_thread = threading.Thread(
        target=monitor_task_progress, args=(task_id, records_dir)
    )
    ...
    try:
        ...
        process = subprocess.Popen(
            ["/bin/bash", script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def run_report_generation(
    task_id, prompt, model, enable_search, search_engine, api_keys
):
    """
    ...
    with open(input_file, "w") as f:
        json.dump(
            {"topic": "", "intent": "", "domain": "", "id": task_id, "prompt": prompt},
            f,
        )
        f.write("\n")
        ...
        if os.path.isdir(task_dir):
            ...
            if os.path.exists(result_file):
                ...
                try:
                    ...
                    return (
                        jsonify({"error": f"Failed to load output file: {str(e)}"}),
                        500,
                    )
            else:
...
def transform_node_to_graph(node, seen_nodes=None, root=False):
    ...
    if "result" in node:
        ...
        for action_name, action_data in node.get("result", {}).items():
            ...
            actions.append(
                {"name": action_name, "result": action_result, "time": action_time}
            )

            # Track the latest action by time
            if not latest_action_name or action_time > node.get("result", {}).get(
                latest_action_name, {}
            ).get("time", ""):
    ...
    def collect_subtasks(current_node, parent_transformed):
        ...
        sorted_tasks = sorted(
            tasks, key=lambda x: int(str(x.get("nid", "0")).split(".")[-1])
        )
        ...
        for task in sorted_tasks:
            ...
            if "result" in task:
                ...
                for action_name, action_data in task.get("result", {}).items():
                    ...
                    # Track the latest action by time
                    if not latest_action_name or action_time > task.get(
                        "result", {}
                    ).get(latest_action_name, {}).get("time", ""):
                        latest_action_name = action_name
                        latest_action_result = action_result
    ...
    try:
        ...
        try:
            ...
            if result:
                pid = int(result)
                print(
                    f"Found Python engine.py process with PID {pid} for task {task_id}"
                )
        ...
        task_storage[task_id][
            "result"
        ] = "Task was stopped by user request before completion."
        ...
        return (
            jsonify({"status": "error", "error": f"Failed to stop task: {str(e)}"}),
            500,
        )
...
def api_delete_task(task_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "ok", "message": f"Task {task_id} deleted successfully"}
        )
    except Exception as e:
        app.logger.error(f"Error deleting task {task_id}: {str(e)}")
        return (
            jsonify({"status": "error", "error": f"Failed to delete task: {str(e)}"}),
            500,
        )
...
def api_ping():
    ...
    return jsonify(
        {"status": "ok", "message": "API server is running", "version": "1.0.0"}
    )
...
def monitor_task_progress(task_id, nodes_dir):
    ...
    try:
        ...
        while task_storage.get(task_id, {}).get("status") not in [
            "completed",
            "error",
            "stopped",
        ]:
            if os.path.exists(nodes_file):
                ...
                if current_modified > last_modified:
                    ...
                    try:
                        ...
                        transformed_graph = transform_node_to_graph(
                            nodes_data, root=True
                        )

                        # Send update via WebSocket
                        print(
                            f"Sending task_update with {len(transformed_graph.get('sub_tasks', []))} sub-tasks"
                        )

                        # Debug output - check if we have action information
                        if "latest_action" in transformed_graph:
                            print(
                                f"Root node has latest action: {transformed_graph['latest_action']['name']}"
                            )

                        # Debug the first subtask if available
                        if (
                            transformed_graph.get("sub_tasks")
                            and len(transformed_graph.get("sub_tasks", [])) > 0
                        ):
                            first_task = transformed_graph["sub_tasks"][0]
                            if "latest_action" in first_task:
                                print(
                                    f"First subtask has latest action: {first_task['latest_action']['name']}"
                                )

                        socketio.emit(
                            "task_update",
                            {"taskId": task_id, "taskGraph": transformed_graph},
                        )
                    except Exception as e:
        ...
        print(
            f"Task {task_id} status changed to {task_storage.get(task_id, {}).get('status')}"
        )
        ...
        if os.path.exists(nodes_file):
            try:
                ...
                transformed_graph = transform_node_to_graph(nodes_data, root=True)
                print(
                    f"Sending final task_update with status {task_storage.get(task_id, {}).get('status')}"
                )
                socketio.emit(
                    "task_update",
                    {
                        ...
                        "status": task_storage.get(task_id, {}).get(
                            "status", "unknown"
                        ),
                    },
                )
...
def handle_connect():
    ...
    socketio.emit(
        "connection_test", {"message": "Connected successfully to the server"}
    )
...
def handle_subscribe(data):
    ...
    if not task_id:
        ...
        emit(
            "subscription_status",
            {"status": "error", "message": "No taskId provided", "taskId": None},
        )
        return
...
if __name__ == "__main__":
    socketio.run(
        app, debug=True, host="0.0.0.0", port=args.port, allow_unsafe_werkzeug=True
    )
