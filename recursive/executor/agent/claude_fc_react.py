...
class SearchAgent(BaseAgent):
    ...
    def __init__(
        ...
        super().__init__(llm=llm, action_executor=action_executor, protocol=None)
    ...
    def parse(self, message):
        ...
        """
        output = {
            key: parse_hierarchy_tags_result(message, tk)
        ...
        try:
            ...
            logger.error(
                "Parse search querys error with output dict: {}".format(output)
            )
    ...
    def format(
        ...
        for turn_info in context_historys:
            ...
            if turn_info.get("tool_result") and isinstance(
                turn_info["tool_result"], dict
            ):
                tool_result_content = turn_info["tool_result"].get(
        ...
        if len(context_historys) > 0:
            if context_historys[-1].get("tool_result") and isinstance(
                context_historys[-1]["tool_result"], dict
            ):
    ...
    def chat(
        ...
        **kwargs,
        ...
        for turn in range(0, self.max_turn):
            ...
            while cnt < 100:
                response = self._llm.call(
                    ...
                    overwrite_cache=True if cnt > 0 else False,
                    **kwargs,
                )[0]["message"]["content"]
                try:
                    ...
                    logger.info(
                        "Meet Exception when parse response: {}, error is : {}".format(
            ...
            action_input = {
                "query_list": current_turn_info["search_querys"],
            ...
            try:
                if (
                    current_turn_info["tool_result"]
                    and "web_pages" in current_turn_info["tool_result"]
                ):
        ...
        return {"ori": return_info, "result": results}
...
{content}
    ...
    react_agent_result = search_agent.chat(
        message=goal,
        global_start_index=global_start_index,
        to_run_target_write_tasks=to_run_target_write_tasks,
        to_run_root_question=to_run_root_question,
