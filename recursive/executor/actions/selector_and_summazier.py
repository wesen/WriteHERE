...
class EvidenceSelectorAPIClientOpenAI:
    def __init__(self, model, max_parralel_requests=10, language="en"):
        """
        ...
        while cnt2 < 5:
            try:
                msg = [
                    {
                        "role": "system",
                        "content": (
                            self.zh_sys_pe if self.language == "zh" else self.en_sys_pe
                        ),
                    },
                ...
                response = self.llm.call(
                    messages=msg,
                    model=self.model,
                    overwrite_cache=(cnt > 0 or cnt2 > 0),
                )
                if response is None:
                ...
                judgement = parse_hierarchy_tags_result(
                    response[0]["message"]["content"], ["answer"]
                )
                ...
                if (
                    "非常丰富" in judgement
                    or "rich and fully satisfy" in judgement.lower()
                ):
                    score = 3
                elif "完全满足" in judgement or "fully satisfy" in judgement.lower():
                    score = 2
                elif (
                    "部分满足" in judgement or "partially satisfy" in judgement.lower()
                ):
                    score = 1
        ...
        for page in web_pages:
            if self.language == "en":
                page["to_run_prompt"] = self.en_prompt.format(
                    query=question, passage=format_web_page(page), to_run_think=think
                )
            else:
...
""".strip()
    ...
    def predict(self, page, temperature=0.01, max_new_tokens=10, do_sample=False):
        ...
        while True:
            try:
                msg = [
                    {
                        "role": "system",
                        "content": (
                            self.en_sys_pe if self.language == "en" else self.zh_sys_pe
                        ),
                    },
                    {"role": "user", "content": page["to_run_prompt"]},
                ]
                response = self.llm.call(
                    messages=msg, model=self.model, overwrite_cache=(cnt > 0)
                )
                ...
                summary = parse_hierarchy_tags_result(
                    response[0]["message"]["content"], ["content"]
                )
    ...
    def summarize(
        ...
        summarizied_web_pages = [
            page
            for page in summarizied_web_pages
            if (
                page["summary"].strip().lower()
                not in (
                    "无内容",
                    "“无内容”",
                    "无内容。",
...
def selector(web_pages, question, think, N, query_list, language, max_workers, model):
    web_pages = EvidenceSelectorAPIClientOpenAI(
        max_parralel_requests=max_workers, language=language, model=model
    ).compute_score(web_pages, question, think, batch_size=max_workers)
    logger.info(
        "Get {} web_pages to select, the result is {}".format(
            len(web_pages),
            ", ".join(map(str, [page["judgement"] for page in web_pages])),
        )
    )
    web_pages = [page for page in web_pages if page["judgement"] != 0]
    web_pages = sorted(
        web_pages, key=lambda x: (x["judgement"], -x["pk_index"]), reverse=True
    )
    ...
    logger.info(
        "After Sort, the Left Result is {}; Each Query Stat: {}".format(
            ", ".join(
                [
                    "{}: {}".format(page["pk_index"], page["judgement"])
                    for page in web_pages
                ]
            ),
            Counter([page["search_query"] for page in selected_pages]).most_common(
                1000
            ),
        )
    )
...
def summarizier(web_pages, question, think, language, max_workers, model):
    web_pages = Summarizier(
        max_parralel_requests=max_workers, language=language, model=model
    ).summarize(web_pages, question, think, batch_size=max_workers)
    return web_pages
...
""".strip()
    summarizier([web_page], "姚明出生那年NBA亚军教练是谁？", think2)
