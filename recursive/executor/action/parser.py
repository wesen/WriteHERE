...
class BaseParser:
    ...
    def __init__(self, action):
        ...
        if action.description:
            for api in action.description.get("api_list", [action.description]):
                name = (
                    f'{action.name}.{api["name"]}'
                    if self.action.is_toolkit
                    else api["name"]
                )
    ...
    def parse_outputs(self, outputs: Any) -> List[dict]:
        ...
        if isinstance(outputs, dict):
            ...
            outputs = str(outputs)
        return [
            {"type": "text", "content": outputs.encode("gbk", "ignore").decode("gbk")}
        ]
...
class TupleParser(BaseParser):
    ...
    def parse_inputs(self, inputs: Union[str, tuple], name: str = "run") -> dict:
        ...
        inputs = {
            self._api2param[name][i]["name"]: item for i, item in enumerate(inputs)
        }
        return inputs
