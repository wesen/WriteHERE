...
class BaseAction(metaclass=AutoRegister(TOOL_REGISTRY, ToolMeta)):
    ...
    def __call__(self, inputs: str, name="run") -> ActionReturn:
        ...
        if isinstance(outputs, ActionReturn):
            ...
            action_return = ActionReturn(inputs, type=self.name, result=outputs)

        # if isinstance(action_return.result, (list, dict)):
        #     import json
        #     action_return.result = json.dumps(action_return.result, ensure_ascii=False)
