...
class OpenAIApiProxy:
    def __init__(self, verbose=True):
        retry_strategy = Retry(
            ...
            status_forcelist=[
                ...
                503,
                504,
            ],  # List of status codes that require retry
    ...
    def call_embedding(self, model, text):
        ...
        headers["Content-Type"] = (
            headers["Content-Type"] if "Content-Type" in headers else "application/json"
        )
        ...
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                ...
                if response.status_code not in self.RETRY_CODES:
                    ...
                    print(
                        f"Received status code {response.status_code} at attempt={attempt + 1}. Retrying..., the reponse is {response.text}",
                        flush=True,
                    )
            ...
            if (
                ...
                print(
                    f"Waiting for {sleep_time} seconds before next attempt...",
                    flush=True,
                )
    ...
    def call(
        ...
        **kwargs,
        ...
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                ...
                if response.status_code not in self.RETRY_CODES:
                    ...
                    if "maximum context length is" in str(
                        response.text
                    ) or "maximum length" in str(response.text):
                        logger.error(
                            "Error Process {} with the maximum context length exceeds. Sys messages is {}".format(
                                model, messages[0]
                            )
                        )
                        # just return None
        ...
        if self.verbose:
            logger.info(
                "Response: {}".format(json.dumps(data, ensure_ascii=False, indent=4))
            )
        ...
        if input_tokens_key in data.get("usage", {}) and output_tokens_key in data.get(
            "usage", {}
        ):
            ...
            # if self.verbose:
            logger.debug(
                "{} input data {}, output_data {} LLM price: {}\n\n".format(
...
if __name__ == "__main__":
    ...
    proxy.call_embedding("text-embedding-3-small", "I am")
