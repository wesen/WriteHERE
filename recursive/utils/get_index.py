...
def extract_and_renumber_citations(text: str, citation_urls):
    ...
    new_citation_urls = {new_num: url for url, new_num in url_to_new_num.items()}
    ...
    def replace_match(match):
        ...
        return (
            f"[reference:{old_to_new[old_num]}]"
...
def process_citations(text, citation_to_url):
    ...
    for cid in sorted(set(used_ids)):
        if cid in citation_to_url:
            ...
            url_to_old_ids[url].append(
                (cid, old2id_2_pos[cid])
            )  # add ori position to sort
